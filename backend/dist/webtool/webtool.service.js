"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.WebtoolService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const webtool_entity_1 = require("./entities/webtool.entity");
const typeorm_2 = require("typeorm");
let WebtoolService = class WebtoolService {
    constructor(webtoolRepository) {
        this.webtoolRepository = webtoolRepository;
    }
    async createWebtool(createWebtoolDto) {
        const existingWebtool = await this.webtoolRepository.findOne({
            where: { webtool: createWebtoolDto.webtool }
        });
        if (existingWebtool) {
            throw new common_1.ConflictException(`Web Tool "${createWebtoolDto.webtool}" already exists`);
        }
        return this.webtoolRepository.save(createWebtoolDto);
    }
    async getAllWebtool() {
        return this.webtoolRepository.find({ order: { 'webtool': 'ASC' } });
    }
    async getWebtoolById(id) {
        const webtool = await this.webtoolRepository.findOne({
            where: { id },
        });
        if (!webtool) {
            throw new common_1.NotFoundException(`Web Tool with ID ${id} not found`);
        }
        return webtool;
    }
    async updateWebtool(id, updateWebtoolDto) {
        const webtool = await this.webtoolRepository.findOne({
            where: { id },
        });
        if (!webtool) {
            throw new common_1.NotFoundException(`Web Tool with ID ${id} not found`);
        }
        if (updateWebtoolDto.webtool && updateWebtoolDto.webtool !== webtool.webtool) {
            const existingWebtool = await this.webtoolRepository.findOne({
                where: { webtool: updateWebtoolDto.webtool }
            });
            if (existingWebtool && existingWebtool.id !== id) {
                throw new common_1.ConflictException(`Web Tool "${updateWebtoolDto.webtool}" already exists`);
            }
        }
        return this.webtoolRepository.save(Object.assign(webtool, updateWebtoolDto));
    }
    async deleteWebtool(id) {
        const webtool = await this.webtoolRepository.findOne({
            where: { id },
        });
        if (!webtool) {
            throw new common_1.NotFoundException(`Web Tool with ID ${id} not found`);
        }
        try {
            await this.webtoolRepository.remove(webtool);
        }
        catch (error) {
            if (error.code === '23503') {
                throw new common_1.ConflictException('Cannot delete Web Tool as it is being used by roles');
            }
            throw error;
        }
    }
};
exports.WebtoolService = WebtoolService;
exports.WebtoolService = WebtoolService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(webtool_entity_1.Webtool)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], WebtoolService);
//# sourceMappingURL=webtool.service.js.map