...
async function loadViteClientCode(file) {
    ...
    const updatedContents = originalContents.replace(/"You can also disable this overlay by setting ",\s*h\("code", \{ part: "config-option-name" \}, "server\.hmr\.overlay"\),\s*" to ",\s*h\("code", \{ part: "config-option-value" \}, "false"\),\s*" in ",\s*h\("code", \{ part: "config-file-name" \}, hmrConfigName\),\s*"\."/, '');
    return updatedContents === originalContents ? originalContents : updatedContents;
}
